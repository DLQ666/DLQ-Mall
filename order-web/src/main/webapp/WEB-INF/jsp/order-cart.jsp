...
<div class="w w1 header clearfix">
    <div id="logo"><a href="/"><img src="/images/zuizhongban.png" alt="奇酷商城"></a></div>
</div>
...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
		...
		<input type="hidden" name="orderShipping.receiverName" value="${userInfo.getCustname()}"/>
		<input type="hidden" name="orderShipping.receiverMobile" value="${userInfo.getCustcall()}"/>
		<input type="hidden" name="orderShipping.receiverState" value="${userAddress.getUserProvince()}"/>
		<input type="hidden" name="orderShipping.receiverCity" value="${userAddress.getUserCity()}"/>
		<input type="hidden" name="orderShipping.receiverDistrict" value="${userAddress.getUserDistrict()}"/>
		<input type="hidden" name="orderShipping.receiverAddress" value="${userAddress.getUserAddress()}"/>
</form>
   			...
   			<ul class="ui-switchable-panel-main" id="consignee-list">
				...
				<li class="ui-switchable-panel" id="consignee_index_137617472"
					...
					<div class="consignee-item item-selected"
						...
						<div class="user-name">
							<div class="fl">
								<strong limit="4">${userInfo.getCustname()}</strong>&nbsp;&nbsp;收
							</div>
							<div class="fr">${userInfo.getCustcall()}</div>
							<div class="clr"></div>
						</div>
						<div class="mt10" limit="15">${userAddress.getUserProvince()} ${userAddress.getUserCity()} ${userAddress.getUserDistrict()} </div>
						<div class="adr-m" limit="30">${userAddress.getUserAddress()}</div>
...
<div class="shopping-list ABTest">
	<div class="goods-list">
     <!--配送方式-->
    <h4 class="vendor_name_h" id="0">商家：奇酷商城</h4>		         
		...
		<div class="mode-tab-nav">
			<ul>
				<li class="mode-tab-item " id="jd_shipment_item"
					...
					id="jdShip-span-tip" class="m-txt">奇酷快递<i
						class='qmark-icon qmark-tip'
						data-tips='由奇酷公司负责配送，速度很快，还接受上门刷卡付款服务'></i></span><b></b></li>
			</ul>
		</div>
	</div>
</div>
    ...
    <div class="ui-ceilinglamp checkout-buttons">
      ...
      <div class="sticky-wrap">
          ...
          <div style="display:none" id="factoryShipCodShowDivBottom" class="dispatching">
            部分商品货到付款方式：先由奇酷配送“提货单”并收款，然后厂商发货。
          </div>
        </div>
